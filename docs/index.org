...
** Docker Registries and Authentication
   ...
   might be surprised when your first ~docker push~ command fails. Because the
   ~push~ command succeeds only when you are successfully authorized by the
   given Docker registry.
...
** Reusable Kubernetes jobs
   ...
   Kubernetes resources (the more complex ones at least) are described by a
   manifest file. The manifest file of a given deployment, pod, service, job
   etc. varies a bit from deployment to deployment that makes the manifest files
   hard to reuse.

   For deployments, services, etc. resource types, we can use Helm packages that
...
* Building Dockerfiles in Kubernetes
  ...
  Dockerfile and potentially other helper files and build it in a Kubernetes
  cluster. Finally, we want to push the resulted Docker image to one or more
  Docker repositories.
...
*** Option 1: Copy credentials from your laptop
    ...
    We can reuse create the Kubernetes secret from this file:
...
*** Starting a job in a reusable way
    ...
    ~yemplate~ let's us to create job manifest templates similarly to Helm
    charts.
    ...
    #+BEGIN_SRC bash :results verbatim :exports both
      yemplate -s "gitrepo=https://github.com/origoss/dockergit.git" \
               ...
               -t https://raw.githubusercontent.com/origoss/k8s-docker-build/master/docker-build.yaml.t
    #+END_SRC
